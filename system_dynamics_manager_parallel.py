...
def instantiate_mfsg_class(inst, m, key):
    """
    Instantiate an MFSG class instance.
    
    This function has to be at the module root to work with concurrency
    (it must be able to be pickled).
    
    Arguments:
        inst {SDM class instance}
        m, key -- usual arguments to init the MFSG class
    
    Returns:
        MFSG instance
    """
    return MeanFieldSolutionGenerator(inst, m, key, solve=False)
...
class SystemDynamicsManager():
    ...
    def __init__(self, params, ncores=None):
        """
        Parameters: params: dict
                        ...
                        files:
                            ...
                            stock_counts.csv
                    ncores: integer
                        The number of cores to use in a parallel job.
                        If  0, 1 or None, run the usual code.
                        If -1, use (system cores)-1.
                        If >1, use that many cores.
        Returns:    None
        """
        ...
        # Parallelization
        if ncores in (0, 1, None):
            self._parallel = False
            self._ncores = None
        else:
            self._parallel = True
            if ncores < 0:
                self._ncores = os.cpu_count()-1 if os.cpu_count() > 1 else 1
            else:
                self._ncores = ncores
        # The time steps.
    ...
    def run(self):
        ...
        if not self.parallel:
            # Run the deterministic model simulations. We set a MFSG instance 
            # for each (sex, race) pair.
            for x, key in enumerate(self.product((2, self.nraces))):
                self.run_sex_race(x, key)
        else:
            # -----------------------------------------------------------------
            # 1. Generate the thetas for each gender-race pair
            # -----------------------------------------------------------------
            # Strange pattern: the future is the dict key so that we can
            # call as_completed() later
            jobs = {}
            with futures.ProcessPoolExecutor(max_workers=self.ncores) as ppe:
                for x, key in enumerate(self.product((2, self.nraces))):
                    m = x / (2 * self.nraces * self.sruns)
                    future = ppe.submit(instantiate_mfsg_class, self, m, key)
                    jobs[future] = key
            # Initialize all the MFSG instances
            for future in futures.as_completed(jobs):
                self.mfsg[jobs[future]] = future.result()

            # -----------------------------------------------------------------
            # 2. Carry out all computations in parallel
            # -----------------------------------------------------------------
            # Parameters: sex_race x foi x runs
            nruns = self.sruns if self.scheck else 1
            ...
            # Initialize the y_alphas to be stored in this class.
            # We cannot mutate the MFG classes in parallel :(
            y_alphas = {}
            for sex_race in self.mfsg:
                y_alphas[sex_race] = {k: [] for k in self.foi_range}
            
            with futures.ProcessPoolExecutor(max_workers=self.ncores) as ppe:
                for sex_race, alpha, _ in params:
                    future = ppe.submit(
                        self.mfsg[sex_race].solve_single_step,
                        alpha-1, alpha)
                    y_alphas[sex_race][alpha] += [future]
                ppe.shutdown(wait=True)

            # -----------------------------------------------------------------
            # 3. Finish up the MFSG solutions
            #    - Collect the y_alphas into their parent instances
            #    - Finalize the solutions
            # -----------------------------------------------------------------
            for sex_race in self.mfsg:
                for alpha in self.foi_range:
                    y_alphas[sex_race][alpha] = [
                        f.result()
                        for f in y_alphas[sex_race][alpha]
                    ]
                self.mfsg[sex_race].finalize_solution(y_alphas[sex_race])
...
class MeanFieldSolutionGenerator():
    ...
    def __init__(self, sdm, m_id, sr, solve=True):
        """
        Parameters: sdm: SystemDynamicsManager()
                    ...
                    solve: bool
                        Run self.solve() automatically? We don't want to do 
                        this when, e.g., we run parallel jobs.
        Returns:    None
        """
    ...
    def solve(self):
        ...
        if not self.sdm.scheck:
            ...
            n_runs = self.sdm.sruns
        
        # Solve for each alpha, storing the y_alpha values
        y_alphas = {}
        for x, alpha in enumerate(self.sdm.foi_range):
            ...
            # Run and collect to self.stored_y_alpha
            for i in range(n_runs):
                if self.sdm.scheck:
                    print('Stochastic run number: {0}'.format(i+1))
                if alpha not in y_alphas:
                    y_alphas[alpha] = []
                y_alphas[alpha] += [self.solve_single_step(x, alpha)]
        
        # Finalize the solutions
        self.finalize_solution(y_alphas)

    def finalize_yalpha(self, y_alphas):
        """
        If deterministic, convert each y_alpha to a single np array. If 
        stochastic, convert the array wrapper to an np array.

        The issue is that we store each y_alpha as a list of arrays. In the 
        stochastic case, this needs to become an np array. In the deterministic 
        case (1 run), we don't need the wrapper at all; each y_alpha should 
        just be an np array.
        ...
        """
        if not self.sdm.scheck:
            # Unwrap the y_alpha solutions
            for a in y_alphas:
                assert len(y_alphas[a]) == 1
                y_alphas[a] = y_alphas[a][0]
        else:
            for a in y_alphas:
                y_alphas[a] = np.array(y_alphas[a])
        return y_alphas
    
    def finalize_solution(self, y_alphas):
        """Take stored y_alpha values and produce a final solution.

        Returns:
            None; sets instance properties
        """

        # Finalize (merge) the y_alphas
        y_alphas = self.finalize_yalpha(y_alphas)

        # Merge all solutions to form sol_0
        sol_0 = None
        for a in y_alphas:
            if sol_0 is None:
                sol_0 = y_alphas[a]
    ...
    def solve_single_step(self, alpha_step, alpha):
        """
        Solve a single alpha step---even a single stochastic run---without 
        doing any of the other work that is requiredto produce a full solution. 
        This enables direct parallelization.
        
        Arguments:
            alpha_step {int} -- the alpha step index; typically alpha-1
            alpha {int} -- an alpha index
        
        Returns:
            a single solution
        """
        if not self.sdm.scheck:
            y_alpha = self.sdm.foi_dist[alpha_step] * self.solve_foi(alpha)
        else:
            y_alpha = self.sdm.foi_dist[alpha_step] * self.solve_sfoi(alpha)
        return y_alpha
